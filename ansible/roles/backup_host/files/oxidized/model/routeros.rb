# MemHamWAN override of the stock Oxidized RouterOS model.
# Placed in <config>/model/ it takes precedence over the built-in.
#
# Why: on RouterOS 7.16 "/export hide-sensitive" does NOT hide
# /routing/ospf/interface-template auth-key (and we don't trust it to catch
# every sensitive attribute on every version), so this adds an explicit
# redaction pass over the export before it is stored/pushed publicly.
# Identical to upstream (2026-08) except for the REDACTED block below.
  ...
  post do
    ...
    cmd run_cmd do |cfg|
      ...
      if vars(:remove_secret)
        # hide-sensitive misses some attributes (OSPF auth-key on 7.16);
        # belt-and-suspenders redaction of anything credential-shaped.
        cfg.gsub!(/\b(auth-key|authentication-key|tcp-md5-key|md5-key|password|secret|secrets|wpa2?-pre-shared-key|pre-shared-key|passphrase|private-key|caller-id-password)=("[^"]*"|\S+)/) do

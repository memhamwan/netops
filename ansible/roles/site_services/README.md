...
| service IPs announced | none | the addresses in `anycast_services` (dns + ntp; the `authdns` pair too when `nsd_enabled`) |
...
- **One dummy interface** (`anycast0`) carries every service /32; each address is
